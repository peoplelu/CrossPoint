...
class CycleNet(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.num_group = self.config.dvae_config.num_group
        self.group_size = self.config.dvae_config.group_size
        self.group_divider = Group(num_group = self.num_group, group_size = self.group_size)
        self.transformer_q = MaskTransformer(self.config)
        self.transformer_q._prepare_encoder(self.config.dvae_config.ckpt)
        self.mixup = config.transformer_config.mixup
    ...
    def forward(self, pts):
        #neighborhood:16, 64, 32, 3  center:16, 64, 3
        neighborhood, center = self.group_divider(pts)
        #q_cls_feature:16 512 logits:16, 64, 8192
        q_cls_feature, logits = self.transformer_q(neighborhood, center, return_all_tokens = self.return_all_tokens)
        q_cls_feature = nn.functional.normalize(q_cls_feature, dim=1)

        if self.mixup:
          mixup_ratio, mixup_neighborhood, mixup_center = self._mixup_pc(neighborhood, center)
